...
`$ARGUMENTS`에 이슈 번호가 있으면 해당 이슈를 사용한다. 없으면 아래 기준으로 자동 선택한다.
...
1. **기존 스펙 우선**: `specs/` 디렉토리에 이미 spec.md가 있는 이슈를 우선한다 (기존 작업 재활용, 중복 방지)
2. **의존성 충족**: 선행 이슈가 완료되었거나 의존성이 없는 것
3. **범위 독립성**: 열린 PR과 충돌하지 않는 것
4. **수행 가능성**: 스펙만으로 구현 가능한 것 (제품 결정 필요 없음)
5. **크리티컬 패스**: 다른 이슈를 언블록하는 것 우선
...
- `build` 스크립트가 있으면 실행한다
- 없으면 `tsc --noEmit`으로 타입 검사만 수행한다
- `lint` 스크립트가 있으면 함께 실행한다
